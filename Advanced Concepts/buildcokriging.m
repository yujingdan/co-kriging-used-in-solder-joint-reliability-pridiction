...
p=2;  % added p definition (February 10)rho=ModelInfo.rho;
rho=ModelInfo.rho;
one=ones(ne+nc,1);
y=[yc; ye];
PsicXc=zeros(nc,nc);
...
[L2,ModelInfo.UPsicXc]=lu_decompose(ModelInfo.PsicXc);
...
% [L3,ModelInfo.UPsicXe]=lu_decompose(ModelInfo.PsicXe);


PsicXcXe=zeros(nc,ne);
for i=1:nc
	for j=1:ne
		PsicXcXe(i,j)=exp(-sum(thetac.*abs(Xc(i,:)-Xe(j,:)).^p)); % build top right corner
	end
end
...
for i=1:ne
	for j=i+1:ne
		PsidXe(i,j)=exp(-sum(thetad.*abs(Xe(i,:)-Xe(j,:)).^p)); % build top right corner
	end
end
ModelInfo.PsidXe=PsidXe+PsidXe'+eye(ne)+eye(ne).*eps;
[L,ModelInfo.UPsidXe]=lu_decompose(ModelInfo.PsidXe);


% ModelInfo.muc=(ones(nc,1)'*(ModelInfo.UPsicXc\(L2\yc)))/(ones(nc,1)'*(ModelInfo.UPsicXc\(L2\ones(nc,1))));
ModelInfo.muc=(ones(nc,1)'*lsqminnorm(ModelInfo.UPsicXc,lsqminnorm(L2,yc)))/(ones(nc,1)'*lsqminnorm(ModelInfo.UPsicXc,lsqminnorm(L2,ones(nc,1))));
% ModelInfo.d=ye-rho.*yc(end-ne+1:end);
%% 
for i=1:ne
    ycxe(i)=pred(ModelInfo.Xe(i,:));
end
ModelInfo.d=ModelInfo.ye-rho.*ycxe';
%% 
ModelInfo.mud=(ones(ne,1)'*(ModelInfo.UPsidXe\(L\ModelInfo.d)))/(ones(ne,1)'*(ModelInfo.UPsidXe\(L\ones(ne,1))));

ModelInfo.SigmaSqrc=(yc-ones(nc,1).*ModelInfo.muc)'*lsqminnorm(ModelInfo.UPsicXc,lsqminnorm(L2,(yc-ones(nc,1).*ModelInfo.muc)))/nc; 
...
% eig(ModelInfo.C)
[ModelInfo.L1,ModelInfo.UC]=lu_decompose(ModelInfo.C);
% 
ModelInfo.mu=(one'*lsqminnorm(ModelInfo.UC,lsqminnorm(ModelInfo.L1,y)))/(one'*lsqminnorm(ModelInfo.UC,lsqminnorm(ModelInfo.L1,one)));
