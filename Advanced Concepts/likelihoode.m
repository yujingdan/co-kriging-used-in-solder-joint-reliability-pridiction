function NegLnLikec=likelihoode(x)
...
[ModelInfo.U19,p]=chol(PsicXc);
if p>0
    NegLnLikec=1000000;
else
LnDetPsicXc=2*sum(log(abs(diag(ModelInfo.U19))));
muc=(one'*(lsqminnorm(ModelInfo.U19,(lsqminnorm(ModelInfo.U19',yc)))))/(one'*(lsqminnorm(ModelInfo.U19,(lsqminnorm(ModelInfo.U19',one)))));
SigmaSqrc=(yc-one.*muc)'*(lsqminnorm(ModelInfo.U19,(lsqminnorm(ModelInfo.U19',(yc-one.*muc)))))/nc;
