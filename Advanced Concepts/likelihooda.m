...
[ModelInfo.U18,p]=chol(PsicXc);
% [ModelInfo.L,ModelInfo.U18]=lu(PsicXc)
if p>0
    NegLnLikec=1000000;
else
LnDetPsicXc=2*sum(log(abs(diag(ModelInfo.U18))));
muc=(one'*(lsqminnorm(ModelInfo.U18,(lsqminnorm(ModelInfo.U18',yc)))))/(one'*(lsqminnorm(ModelInfo.U18,(lsqminnorm(ModelInfo.U18',one)))));
SigmaSqrc=(yc-one.*muc)'*(lsqminnorm(ModelInfo.U18,(lsqminnorm(ModelInfo.U18',(yc-one.*muc)))))/nc;
NegLnLikec=-1*(-(nc/2)*log(SigmaSqrc)-0.5*LnDetPsicXc);
% muc=(one'*(ModelInfo.U18\((ModelInfo.L\yc))))/(one'*(ModelInfo.U18\(ModelInfo.L\one)));
% SigmaSqrc=(yc-one.*muc)'*(ModelInfo.U18\(ModelInfo.L\(yc-one.*muc)))/nc;
% NegLnLikec=-1*(-(nc/2)*log(SigmaSqrc)-0.5*LnDetPsicXc);
end
