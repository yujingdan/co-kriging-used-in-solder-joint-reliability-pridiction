...
y=[yc; ye];
% v=size(rho*ModelInfo.SigmaSqrc*exp(-sum(thetac.*abs(Xc(1,:)-x).^p)))
cc=ones(nc,1);
for i=1:nc
	cc(i)=rho*ModelInfo.SigmaSqrc*exp(-sum(thetac.*abs(Xc(i,:)-x).^p));
end
cd=ones(ne,1);
% s=size(rho^2*ModelInfo.SigmaSqrc*exp(-sum(thetac.*abs(Xe(1,:)-x).^p))+ModelInfo.SigmaSqrd*exp(-sum(thetad.*abs(Xe(1,:)-x).^p)))
for i=1:ne
	cd(i)=rho^2*ModelInfo.SigmaSqrc*exp(-sum(thetac.*abs(Xe(i,:)-x).^p))+ModelInfo.SigmaSqrd*exp(-sum(thetad.*abs(Xe(i,:)-x).^p));
end
c=[cc;cd];
% f=ModelInfo.mu+c'*(ModelInfo.UC\(y-one.*ModelInfo.mu)); 
f=ModelInfo.mu+c'*(lsqminnorm(ModelInfo.C,(y-one.*ModelInfo.mu))); 
if strcmp(ModelInfo.Option,'Pred')==0
    SSqr=rho^2*ModelInfo.SigmaSqrc+ModelInfo.SigmaSqrd-c'*(ModelInfo.UC\(ModelInfo.L1\c));
    s=abs(SSqr)^0.5;
    if strcmp(ModelInfo.Option,'RMSE')==0
            ModelInfo.yBest=min(ye);
        if strcmp(ModelInfo.Option,'NegProbImp')==1
            ProbImp=0.5+0.5*erf((1/sqrt(2))*((ModelInfo.yBest-f)/s));
        else
            EITermOne=(ModelInfo.yBest-f)*(0.5+0.5*erf((1/sqrt(2))*((ModelInfo.yBest-f)/s)));
            EITermTwo=s*(1/sqrt(2*pi))*exp(-(1/2)*((ModelInfo.yBest-f)^2/SSqr));
